...
echo '--- GET after POST ---'
curl -s -b "$COOKIE" http://app/forgot-password | grep -oE 'flash-(success|info|error)[^<]*<div class="flash-body">[^<]+' | head -n 10
echo '--- Reset URL present? ---'
curl -s -b "$COOKIE" http://app/forgot-password | grep -oE 'reset-password\?token=[a-f0-9]+' | head -n 3
